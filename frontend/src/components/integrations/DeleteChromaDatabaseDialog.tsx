...
import { ChromaDatabaseInfo } from "@/lib/api";

interface DeleteChromaDatabaseDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  database: ChromaDatabaseInfo | null;
  loading?: boolean;
}

export function DeleteChromaDatabaseDialog({
  isOpen,
  onClose,
  onConfirm,
  database,
  loading = false,
}: DeleteChromaDatabaseDialogProps) {
      ...
      <div className="bg-flextide-neutral-panel-bg rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="p-6 border-b border-flextide-neutral-border">
          <h2 className="text-xl font-semibold text-flextide-neutral-text-dark">
            Delete Chroma Database Connection
          </h2>
        ...
        <div className="p-6">
          <p className="text-sm text-flextide-neutral-text-dark mb-4">
            Are you sure you want to delete the Chroma database connection{" "}
            <span className="font-medium">"{database.name}"</span>?
            This will permanently remove the connection and it will no longer be available for use in workflows.
          </p>
          ...
          <div className="bg-flextide-error/10 border border-flextide-error rounded-md p-3 mb-4">
            <p className="text-xs text-flextide-error">
              <strong>Warning:</strong> This action cannot be undone. The database connection will be permanently deleted.
            </p>
          </div>
