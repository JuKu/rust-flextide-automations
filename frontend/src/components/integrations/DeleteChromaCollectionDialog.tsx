...
import { ChromaCollectionInfo } from "@/lib/api";

interface DeleteChromaCollectionDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export function DeleteChromaCollectionDialog({
  isOpen,
  onClose,
  onConfirm,
  collection,
  loading = false,
}: DeleteChromaCollectionDialogProps) {
      ...
      <div className="bg-flextide-neutral-panel-bg rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div className="p-6">
          <p className="text-sm text-flextide-neutral-text-dark mb-4">
            ...
            <span className="font-medium">&quot;{collection.name}&quot;</span> from database{" "}
            <span className="font-medium">&quot;{collection.database_name}&quot;</span>?
            This will permanently remove the collection and all its documents.
          </p>
          ...
          <div className="bg-flextide-error/10 border border-flextide-error rounded-md p-3 mb-4">
            <p className="text-xs text-flextide-error">
              <strong>Warning:</strong> This action cannot be undone. The collection and all its documents will be permanently deleted.
            </p>
          </div>
