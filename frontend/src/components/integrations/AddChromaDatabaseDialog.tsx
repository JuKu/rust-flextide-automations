...
import { createChromaDatabase, testChromaConnection, ChromaCredentials, CreateChromaDatabaseRequest } from "@/lib/api";
import { showToast } from "@/lib/toast";

interface AddChromaDatabaseDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function AddChromaDatabaseDialog({
  isOpen,
  onClose,
  onSuccess,
}: AddChromaDatabaseDialogProps) {
  ...
  useEffect(() => {
    function handleEscape(e: KeyboardEvent) {
      if (e.key === "Escape" && !loading && !testingConnection) {
  ...
  }, [isOpen, loading, testingConnection, onClose]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const request: CreateChromaDatabaseRequest = {
        name: name.trim(),
        credentials,
      };

      await createChromaDatabase(request);
      showToast("Chroma database connection created successfully", "success");
      onSuccess();
    } catch (err) {
      console.error("Failed to create Chroma database:", err);
      const errorMessage = err instanceof Error ? err.message : "Failed to create Chroma database";
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
      ...
      >
        <div className="p-6 border-b border-flextide-neutral-border">
          ...
          <p className="text-sm text-flextide-neutral-text-medium mt-1">
            Configure a new Chroma vector database connection
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              required
              disabled={loading || testingConnection || isSecuredFieldsDisabled}
            >
          ...
          <div className="flex justify-end gap-3">
            ...
            <div className="relative group">
              <button
                type="submit"
                disabled={loading || testingConnection || !connectionTested}
              ...
              {!connectionTested && !loading && !testingConnection && (
